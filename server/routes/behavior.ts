...
function calculateRisk(report: Record<string, unknown>) {
  let score = 0;
  ...
  const mood = (typeof report.mood === 'string' ? report.mood : "").toLowerCase();
...
router.post('/behavior/report', async (req, res) => {
  try {
    ...
    let report;
    try {
        report = await prisma.behaviorReport.create({
            data: {
        ...
        console.error("Prisma Report Error:", reportError);
    ...
    if (risk === 'medium' || risk === 'high') {
      ...
      try {
        await prisma.flaggedCase.create({
          ...
          console.error("Prisma Flagged Case Error:", caseError);
      }
  ...
  try {
    ...
    try {
        cases = await prisma.flaggedCase.findMany({
            where: { intervention_status: { not: 'resolved' } },
            orderBy: { created_at: 'desc' }
        });
        res.json(cases);
    } catch (error) {
        console.error("Prisma Fetch Cases Error:", error);
...
router.get('/cases/:id', volunteerOnly, async (req: AuthRequest, res: Response) => {
  try {
    const c = await prisma.flaggedCase.findUnique({
    ...
    if (c.report_id) {
        reportContext = await prisma.behaviorReport.findUnique({
    ...
    if (c.session_id) {
        sessionContext = await prisma.session.findUnique({
            ...
            select: { user_name: true, age_group: true, phone: true }
        });
    }
...
router.patch('/cases/:id/status', volunteerOnly, async (req: AuthRequest, res: Response) => {
  try {
    ...
    try {
        const updated = await prisma.flaggedCase.update({
            where: { id: req.params.id as string },
            data: { intervention_status: status }
        });
        res.json(updated);
    } catch {
