    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-brand-bg relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-md w-full bg-white rounded-[2.5rem] shadow-premium p-8 md:p-10 scale-in shadow-brand-navy/5">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          <div className="w-16 h-16 rounded-3xl bg-brand-bg text-brand-navy flex items-center justify-center mb-4 shadow-sm border border-slate-100">
            <Lock size={28} />
          </div>
          <h1 className="text-2xl font-black text-brand-navy tracking-tight">Safe Chat Login</h1>
          <p className="text-sm text-text-muted mt-2 leading-relaxed">
            Access your secure support history.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-brand-navy hover:bg-slate-800 text-white font-black py-4 rounded-2xl shadow-xl shadow-brand-navy/20 transition-all transform hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-2 group disabled:opacity-50"
          >
            {loading ? "Signing in..." : "Access Safe Space"}
            {!loading && <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />}
          </button>
        </form>

        <p className="mt-6 text-center text-sm font-medium text-text-muted">
          Don&apos;t have a Safe Chat account?{" "}
          <Link href="/signup" className="text-brand-teal hover:text-teal-700 underline underline-offset-4 font-bold">
            Create Profile
          </Link>
        </p>
      </div>
